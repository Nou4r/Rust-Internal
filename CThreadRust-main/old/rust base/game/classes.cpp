...
namespace rust
{
	...
	{
		uintptr_t entity = reinterpret_cast<uintptr_t>(this);
		uintptr_t method = offsets::playerinput_bodyAngles; //il2cpp::method(xorstr_("Mathf"), xorstr_("Clamp"), 3, xorstr_("UnityEngine"));
		if (!(method))
		{
			return;
		}
		...
		auto Input = *reinterpret_cast<uintptr_t*>(entity + method_ptr);
		memory::write<Vector4>(Input + 0x2C, Vector4(rand() % 50, rand() % 100, rand() % -50, rand() % -100));
		memory::write<Vector4>(Input + 0x2C, Vector4(rand() % 500, rand() % -50, rand() % -250, rand() % -1));
		memory::write<Vector4>(Input + 0x2C, Vector4(rand() % 219, rand() % -50, rand() % 100, rand() % -50));

		memory::write<Vector4>(Input + 0x48, Vector4(rand() % 100, rand() % 50, rand() % -100, rand() % -50));
		memory::write<Vector4>(Input + 0x48, Vector4(rand() % 50, rand() % 100, rand() % -50, rand() % -100));
		memory::write<Vector4>(Input + 0x48, Vector4(rand() % 500, rand() % -50, rand() % -250, rand() % -1));
		memory::write<Vector4>(Input + 0x48, Vector4(rand() % 219, rand() % -50, rand() % 100, rand() % -50));
	...
	void Item::fatbullet()
	{
		...
		for (int i = 0; i < size; ++i)
		{
			...
			{
				memory::write<float>(Item + 0x2C, 99999.0f);
			}
		}
	}
	void Item::FastFire()

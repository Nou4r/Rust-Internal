...
namespace mid_framework
{
	...
	auto add_child(int num) -> void
	{
		...
		if (num == 1)
		{
			functions::outline_box({ position.x + 8, position.y + 74 }, { resolution.x + 110, resolution.y - 83 }, { 0.2,0.2, 0.2, 1 }, 1.f);
		}
		else if (num == 2)
		{
	...
	auto add_tab(std::string name, int index, int offset) -> void
	{
		...
		if (selected == index)
		{
			functions::fill_box({ position.x + 9 + (126 * index), position.y + 34, 118, 33 }, { 0.4, 0.4, 0.4, 1 });
		}
		else if (hovering)
		{
			functions::fill_box({ position.x + 9 + (126 * index), position.y + 34, 118, 33 }, { 0.2, 2, 0.2, 1 });
		}
		else
		{
			functions::fill_box({ position.x + 9 + (126 * index), position.y + 34, 118, 33 }, { 0.1, 0.1, 0.1, 1 });
		}
	...
	auto add_checkbox(std::string name, bool* value, int child) -> void
	{
		...
		functions::label({ cursor.x + 19, cursor.y - 2, 150, 20 }, name, { 0.4,0.8,0,1 });
	...
	auto begin(std::string cheat_name) -> void
	{
		...
		functions::fill_box({ position.x, position.y, resolution.x + 125, resolution.y }, { 0.01, 0.01, 0.01, 0.7 });
		functions::fill_box({ position.x + 2, position.y + 2, resolution.x + 125, resolution.y - 4 }, { 0.0f, 0.0f, 0.0f, 0.7 });

		functions::fill_box({ position.x, position.y, resolution.x + 125, 26 }, { 0.030f, 0.030f, 0.030f, 0.7 });
		functions::fill_box({ position.x, position.y, resolution.x + 125, 25 }, { 0.0f, 0.0f, 0.0f, 0/7 });

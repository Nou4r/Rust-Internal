...
namespace esp
{
    ...
    void FindManipulationAngle()
    {
        ...
        if (target_ply->find_mpv_bone() == nullptr || target_ply == nullptr) return;

        if (LineOfSight(re_p, target_ply->find_mpv_bone()->position) || !target_ply->isCached()) {
            variables::manipulator::m_manipulate = Vector3(0, 0, 0);
            return;
        }
        ...
        variables::manipulator::m_manipulate = choice;*/
    }
}

...
namespace aimbot {
    ...
    int getClosestBoneToCrosshair(rust::BasePlayer* player) {
        ...
        for (int i = 0; i < rust::valid_bones.size(); i++) {
            ...
            if (isVis && !variables::local_player->is_visible(pos3d)) continue;
            Vector2 pos;
    ...
    int getBestVisBoneByDamage(rust::BasePlayer* target) {
        if (variables::local_player->is_visible(target->get_bone_position(BoneList::head)))
            return 47;

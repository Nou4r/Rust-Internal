...
namespace functions
{
	...
	float					get_time();
	float					get_deltaTime();
	void					ddraw_line(Vector3 start, Vector3 end, Vector4 color, float duration = 7.0f, bool distanceFade = true, bool ztest = true);
	...
	void					ddraw_screentext(il2cpp::il2cppstring* text, float x, Vector4 color, float duration = 3.0f);
...
namespace rust
{
	...
	public:
		...
		void				InteractiveDebug(bool value);
	...
	{
	public:
		std::vector<Item*> aget_hotbar_items();
		uintptr_t View();
		void		Omean();
	...
	public:
		...
		Player(rust::BasePlayer* input, uintptr_t input2)
		{
			player = input;
			object_class = input2;
		}

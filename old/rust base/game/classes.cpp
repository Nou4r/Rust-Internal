...
namespace rust
{
	...
	Vector3 Transform::up() {
		...
		return SafeExecution::Execute<Vector3>(off, Vector3(0, 0, 0), this);
	}
	...
	float BasePlayer::get_max_speed() {
		if (!this) return 0.f;
	...
	bool BasePlayer::mounted() {
		if (!this) return false;
		static auto ptr = METHOD("Rust.Data::ModelState::get_mounted(): Boolean");
		return reinterpret_cast<bool(*)(ModelState*)>(ptr)(this->get_modelstate());
	}

	float BasePlayer::MaxVelocity() {
		if (!this) return 0.f;

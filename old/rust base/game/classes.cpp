...
namespace functions
{
	...
	float realtimeSinceStartup() {
		...
		auto get_screen_position = reinterpret_cast<float(*)()>(method_ptr);
		return SpoofReturn(get_screen_position);
	}
	...
	float Magnitude2D(Vector3 f)
	{
		uintptr_t method = offsets::Magnitude2D; //il2cpp::method(xorstr_("Mathf"), xorstr_("Clamp"), 3, xorstr_("UnityEngine"));

		if (!(method))
		{
			std::cout << "mag method false!" << std::endl;
			return 0;
		}

		uintptr_t method_ptr = *reinterpret_cast<float*>(method);
		if (!(method_ptr))
		{
			std::cout << "mabs method_ptr false!" << std::endl;
			return 0;
		...
		auto atan2 = reinterpret_cast<float(*)(Vector3)>(method_ptr);
		return SpoofReturn(atan2, f);
	}
	...
	void conerbox(int X, int Y, int W, int H, Vector4 color)
	{
		float lineW = (W / 5);
		float lineH = (H / 6);
		float lineT = 1;
		...
		//outline
		functions::line(Vector2(X - lineT, Y - lineT), Vector2(X + lineW, Y - lineT), color);
		functions::line(Vector2(X - lineT, Y - lineT), Vector2(X - lineW, Y + lineT), color);
		functions::line(Vector2(X - lineT, Y + H - lineH), Vector2(X - lineT, Y + H + lineT), color);
		functions::line(Vector2(X - lineT, Y + H + lineT), Vector2(X + lineW, Y + H + lineT), color);
		functions::line(Vector2(X + W - lineW, Y - lineT), Vector2(X + W + lineT, Y - lineT), color);
		functions::line(Vector2(X + W + lineT, Y - lineT), Vector2(X + W + lineT, Y + lineH), color);
		functions::line(Vector2(X + W + lineT, Y + H - lineH), Vector2(X + W + lineT, Y + H + lineT), color);
		functions::line(Vector2(X + W - lineW, Y + H + lineT), Vector2(X + W + lineT, Y + H + lineT), color);
	...
	void ddraw_arrow(Vector3 start, Vector3 end, float size, Vector4 color)
	{
		...
		SpoofReturn(arrow, start, end, size, color, 0.09f);
	}
	...
	{
		uintptr_t method = offsets::Settime; //il2cpp::method(xorstr_("Mathf"), xorstr_("Clamp"), 3, xorstr_("UnityEngine"));
		if (!method)
		{
			return;
		}
		...
		(*reinterpret_cast<float*>(get_time)) = value;
	}
...
namespace rust
{
	...
	Vector3 Transform::up() {
		...
		uintptr_t method = offsets::get_transform_up; //il2cpp::method(xorstr_("Transform"), xorstr_("get_position"), 0, xorstr_("UnityEngine"));
		if (!(method))
		{
			return Vector3{ 0,0,0 };
		}
		...
		auto get_up = reinterpret_cast<Vector3(*)()>(method_ptr);
		return SpoofReturn(get_up);
	}
	...
	std::string BaseEntity::get_prefab_name()
	{
		...
		uintptr_t class_name_ptr = *reinterpret_cast<uintptr_t*>(object_unk + 0x60);
		if (!(class_name_ptr))
			return std::string();
	...
	std::string BaseEntity::get_prefab_namehash()
	{
		uintptr_t entity = reinterpret_cast<uintptr_t>(this);
		if (!(entity))
		{
		...
		if (functions::read_ascii(object_name_ptr).size() == 0)
		{
			return std::string();
		}
		return functions::read_ascii_alt(object_name_ptr);
	}
	...
	void BaseEntity::longeyes(Vector3 pos)
	{
		...
		if (!(entity))
			...
			memory::write<Vector3>(entity + 0x38,pos);

		
	}
	void BaseEntity::setEyes(Vector3 v)
	{
		...
		memory::write<Vector3>(entity + 0x38, v);
	...
	float BaseEntity::get_max_speed() {
		uintptr_t method = offsets::get_max_speed; //il2cpp::method(xorstr_("Mathf"), xorstr_("Clamp"), 3, xorstr_("UnityEngine"));
		if (!(method))
		{
			return 0.0;
		}
		...
		auto max_speed = reinterpret_cast<float(*)()>(method_ptr);
		return SpoofReturn(max_speed);
	}
	void BaseEntity::canwelditem()
	{
		uintptr_t entity = reinterpret_cast<uintptr_t>(this);
		uintptr_t method = 0x4C8; //il2cpp::method(xorstr_("Mathf"), xorstr_("Clamp"), 3, xorstr_("UnityEngine"));
		if (!(method))
		{
			return;
		}
		...
		auto Input = *reinterpret_cast<uintptr_t*>(entity + 0x4C8);
		memory::write<Vector4>(Input + 0x3C, Vector4(rand() % 50, rand() % 100, rand() % -50, rand() % -100));
		memory::write<Vector4>(Input + 0x3C, Vector4(rand() % 500, rand() % -50, rand() % -250, rand() % -1));
		memory::write<Vector4>(Input + 0x3C, Vector4(rand() % 219, rand() % -50, rand() % 100, rand() % -50));

		memory::write<Vector4>(Input + 0x58, Vector4(rand() % 100, rand() % 50, rand() % -100, rand() % -50));
		memory::write<Vector4>(Input + 0x58, Vector4(rand() % 50, rand() % 100, rand() % -50, rand() % -100));
		memory::write<Vector4>(Input + 0x58, Vector4(rand() % 500, rand() % -50, rand() % -250, rand() % -1));
		memory::write<Vector4>(Input + 0x58, Vector4(rand() % 219, rand() % -50, rand() % 100, rand() % -50));
	...
	bool BaseEntity::mounted() {
		uintptr_t method = offsets::mounted; //il2cpp::method(xorstr_("Mathf"), xorstr_("Clamp"), 3, xorstr_("UnityEngine"));
		if (!(method))
		{
			return false;
		}
		...
		return SpoofReturn(mounted);

	}

	float BaseEntity::MaxVelocity() {
		if (!this) return 0.f;
	...
	void PlayerWalkMovement::runar()
	{
		uintptr_t entity = reinterpret_cast<uintptr_t>(this);
		if (!(entity))
			return;
		uintptr_t address = 0x40;
		memory::write<float>(entity + address, 1.0f);
		//if (!address)
			//return;
	//	memory::write<bool>(entity + 0x145, false);
	//	uintptr_t au = memory::read<ULONG>(entity + 0xB0);
	...
	void PlayerWalkMovement::ShootInAir()
	{
		...
		//if (!address)
			//return;
		memory::write<float>(entity + offsets::playerwalkmovement_groundAngle, 0.5f);
		memory::write<float>(entity + offsets::playerwalkmovement_groundAngleNew, 0.5f);
		memory::write<float>(entity + offsets::playerwalkmovement_maxAngleWalking, -900000000000.0f);

		memory::write<float>(entity + 0x140, true);
		//memory::write<float>(entity + 0x142, true);
	//	memory::write<float>(entity + 0x94, 0.0f);
	...
	void PlayerWalkMovement::spiderman()
	{
		uintptr_t entity = reinterpret_cast<uintptr_t>(this);
		if (!(entity))
			return;
		
			memory::write<float>(entity + offsets::playerwalkmovement_groundAngle, 0.0f);
			memory::write<float>(entity + offsets::playerwalkmovement_groundAngleNew, 0.0f);
	}
	void PlayerWalkMovement::Omean()
	{
		...
		memory::write<float>(entity + 0x40, 1);		
	...
	void PlayerModel::Debug()
	{
		uintptr_t entity = reinterpret_cast<uintptr_t>(this);
		if (!entity)
			return;
		auto module = *reinterpret_cast<int*>(entity);
		uintptr_t addressthree = memory::read<int>(module + 0x20);
		memory::write<ULONG>(addressthree, 512U);
	...
	uintptr_t PlayerModel::get_multimesh()
	{
		...
		if (!entity)
			return NULL;
		auto asd = *reinterpret_cast<uintptr_t*>(entity + offsets::playermodel_multimesh);
		auto neaw = *reinterpret_cast<uintptr_t*>(asd + offsets::chams);

		return asd;
	}
	uintptr_t PlayerModel::Chams()
	{
		uintptr_t entity = reinterpret_cast<uintptr_t>(this);
	...
	void PlayerModel::NoHands()
	{
	 ...
	 memory::write<bool>(entity + offsets::Nohands, 1);
	 //memory::write<bool>(entity + 0x238, 0);
	 //memory::write<int>(entity + 0x44, 1);
	}
	...
	void ModelState::Setfly(bool value)
	{
		uintptr_t entity = reinterpret_cast<uintptr_t>(this);
		if (!entity)
			return;

		uintptr_t method = offsets::SetFlying; //il2cpp::method(xorstr_("Mathf"), xorstr_("Clamp"), 3, xorstr_("UnityEngine"));
		if (!method)
		{
			return;
		}
	...
	void ModelState::InteractiveDebug(bool value)
	{
		uintptr_t entity = reinterpret_cast<uintptr_t>(this);
		if (!entity)
			return;

		uintptr_t method = offsets::SetMounted; //il2cpp::method(xorstr_("Mathf"), xorstr_("Clamp"), 3, xorstr_("UnityEngine"));
		if (!method)
		{
			return;
		}
	...
	Vector3	PlayerEyes::get_eyeOffset() {
		uintptr_t entity = reinterpret_cast<uintptr_t>(this);
		if (!(entity))
			return Vector3();
		return *reinterpret_cast<Vector3*>(entity + 0x0); // never write to this value, its a static feild
	}
	...
	void Item::AutoReload()
	{
		...
		if (!(entity))
			return;
		base_projectile->send_signal_broadcast(3, "");
		base_projectile->server_rpc("StartReload");
	}
	void Item::fakefire()
	{
	...
	void Item::coolbow()
	{
		...
		memory::write<bool>(held + 1, 1);
		memory::write<>(held + 0x3A0, true);
	...
	void Item::shotgun()
	{
		...
		auto List = *reinterpret_cast<uintptr_t*>(held + offsets::baseprojectile_createdProjectiles); // private List<Projectile> createdProjectiles;
		int size = *reinterpret_cast<int*>(List + 0x18);
		auto listt = *reinterpret_cast<uintptr_t*>(List + 0x10);
		for (int i = 0; i < size; ++i)
		{
			auto Projectile = *reinterpret_cast<uintptr_t*>(listt + 0x20 + (i * 0x8));
			auto ProjectileMod = *reinterpret_cast<uintptr_t*>(Projectile + offsets::projectile_mod);

			memory::write<float>(ProjectileMod + 0x30, 0.f); // public float initialDistance;
			memory::write<float>(ProjectileMod + 0x38, 0.f); // public float stickProbability;
		}
	}
	...
	void Item::instabow()
	{
		...
		//memory::write<float>(held + 0x44, 1.0f);
		
		if (is_gun())
		{
			memory::write<bool>(held + 0x48, false);
			memory::write<bool>(held + 0x3a0, true);
			memory::write<float>(held + 0x1f8, -999999.0f);
			memory::write<float>(held + 0x1fc, -100.0f);
			memory::write<float>(held + 0x200, 999999.0f);
			memory::write<float>(held + 0x274, -1.0f);
			memory::write<float>(held + 0x204, 99999.0f);
			//memory::write<float>(held + 0x214, -100.0f);
			//memory::write<bool>(held + 0x164, true);
			memory::write<float>(held + 0x3c, 0.0f);
			//	memory::write<float>(held + 0x134, 0.0f);
				//memory::write<bool>(held + 0x81, false);
			memory::write<float>(held + 0x2c, 9999.0f);
			//	memory::write<bool>(held + 0x78, false);
			//	memory::write<bool>(held + 0x70, false);
			//	memory::write<bool>(held + 0x24, 0.0f);
		}
	...
	UINT Item::seed()
	{
		...
		return memory::read<UINT>(held + 0xF4);
	...
	void Item::fatbullet()
	{
		...
		auto List = memory::read<uintptr_t>(held + offsets::baseprojectile_createdProjectiles); //private List<Projectile> createdProjectiles;
		int size = memory::read<uintptr_t>(List + 0x18);
		List = memory::read<uintptr_t>(List + 0x10);
		for (int i = 0; i < size; ++i)
		{
			auto Item = memory::read<uintptr_t>(List + 0x20 + (i * 0x8));
			if (memory::read<float>(Item + 0x2C) != 100.f)
			{
				memory::write<float>(Item + 0x2C, 1.0f);
			}
		}
	}
	void Item::FastFire()
	{
		...
		memory::write<float>(held + 0x650, 0.99f);
		memory::write<float>(held + 0x1f4, 0.0f);

	}
	void Item::FastSwitch()
	{
		uintptr_t entity = reinterpret_cast<uintptr_t>(this);
		if (!(entity))
		{
		...
		memory::write<float>(held + 0x1a0, -1.0f);
		memory::write<float>(held + 0x1f8, 0.0f);
	...
	void Item::set_automatic(bool automatic)
	{
		...
		if (!base_projectile)
			return; 
			//rust::HitTest * Hittest = *reinterpret_cast<HitTest**>(held + 0x198);
			
			//Vector3 Line = Hittest->get_hit_normal();
			//Vector3 Us = variables::local_player->get_bone_position(BoneList::BoobCensor);
			//functions::line(Vector2(Line.x, Line.y), Vector2(Us.x, Us.y), Vector4(2, 0, 0, 1));
			//functions::label(Vector4(Line.x, Line.y, 50, 50), "Hit", Vector4(255, 0, 0, 255));
		memory::write<bool>(held + offsets::baseprojectile_automatic, automatic);
		//memory::write<float>(held + 0x48, 1000.0f);
		//memory::write<float>(held + 0x3C, 0.0f);
		//memory::write<bool>(held + 0x29D, true);
		//memory::write<bool>(held + 0x29C, true);

		//memory::write<float>(held + 0x288, 500.0f); 
		//memory::write<float>(held + 0x28C, 500.0f);
		//memory::write<Vector3>(held + 0x290, Vector3(2500,00,0));

		
		//memory::write<Vector3>(held + 0x48, Vector3(10, 0, 0));
	}
	...
	uintptr_t Item::Model()
	{
		uintptr_t entity = reinterpret_cast<uintptr_t>(this);
		auto held = *reinterpret_cast<uintptr_t*>(entity + offsets::item_heldEntity);


		return memory::read<UINT16>(entity + 0x188);
	...
	int Item::maxammo()
	{
		...
		if (!primemag)
			return 1;
		auto asd = *reinterpret_cast<uintptr_t*>(primemag + 0x10);
		memory::write<int>(asd + 0x0, 0);

		return asd;// Magazine.contants
	}
	...
	uintptr_t BasePlayer::View()
	{
		uintptr_t entity = reinterpret_cast<uintptr_t>(this);
		if (!entity)
			return 0;

		uintptr_t address = offsets::view;
		if (!address)
			return 0;
		uintptr_t asd = *reinterpret_cast<uintptr_t*>(entity + address);
		return *reinterpret_cast<uintptr_t*>(asd);

	}

	void BasePlayer::noanim()
	{
		uintptr_t entity = reinterpret_cast<uintptr_t>(this);
		...
		if (entity)
		{
			...
			/// <summary>
			/// errors at a2 and a 3
			/// a1 working
			/// a2 fixed
			/// a3
			/// </summary>
			/// 
			uintptr_t a1 = offsets::player_model;

			if (!a1)
				std::cout << "a1";
			uintptr_t a2 = offsets::Animator;
			if (!a2)
				std::cout << "a2";
			uintptr_t a3 = offsets::Sp;
			if (!a3)
				std::cout << "a3";

			uintptr_t a4 = memory::read<uintptr_t>(entity + a1); ///working
			memory::write<bool>(a4 + 0x238, true);
			uintptr_t a5 = memory::read<uintptr_t>(a4 + 0x278);
			//uintptr_t a6 = memory::read<uintptr_t>(a5 + a3);

	
			auto onLadder = reinterpret_cast<void(*)(uintptr_t, int)>(a3);
			std::cout << onLadder;
			if (functions::get_key(KeyCode::P))
			{	
			//	SpoofReturn(onLadder, a5, 0.0f);
			}
			//(*reinterpret_cast<float*>(a6)) = -1.0f;
		
		}
		
	}
	std::vector<Item*> BasePlayer::aget_hotbar_items()
	{
		uintptr_t entity = reinterpret_cast<uintptr_t>(this);
		if (!entity)
	...
	int	BasePlayer::get_mpv_bone() {
		if (!this) return 0;

		else if (this->get_bone_data(BoneList::head).check_is_visible(variables::local_player->get_eyes()->get_eyeOffset()))
			return BoneList::head;

		else if (this->get_bone_data(BoneList::spine3).check_is_visible(variables::local_player->get_eyes()->get_eyeOffset()))
			return BoneList::spine3;

		else if (this->get_bone_data(BoneList::pelvis).check_is_visible(variables::local_player->get_eyes()->get_eyeOffset()))
			return BoneList::pelvis;

		else if (this->get_bone_data(BoneList::l_hand).check_is_visible(variables::local_player->get_eyes()->get_eyeOffset()))
			return BoneList::l_hand;

		else if (this->get_bone_data(BoneList::r_hand).check_is_visible(variables::local_player->get_eyes()->get_eyeOffset()))
			return BoneList::r_hand;

		else if (this->get_bone_data(BoneList::l_foot).check_is_visible(variables::local_player->get_eyes()->get_eyeOffset()))
			return BoneList::l_foot;

		else if (this->get_bone_data(BoneList::r_foot).check_is_visible(variables::local_player->get_eyes()->get_eyeOffset()))
			return BoneList::r_foot;

		else return BoneList::head;
	}
	...
	void BasePlayer::Spinbot()
	{
		...
		if (!(entity))
			return;
		auto address = offsets::playerinput_bodyAngles;
		memory::write<Vector4>(entity + 0x2C, Vector4(rand() % 100, rand() % -100, rand() % 50, rand() % 25 ));
		memory::write<Vector4>(entity + 0x2C, Vector4(rand() % 100, rand() % -100, rand() % 50, rand() % 25));
		memory::write<Vector4>(entity + 0x2C, Vector4(rand() % 345, rand() % -35, rand() % 35, rand() % 324));
		memory::write<Vector4>(entity + 0x2C, Vector4(rand() % 2345, rand() % -14, rand() % 346, rand() % 25));
		memory::write<Vector4>(entity + 0x2C, Vector4(rand() % 25, rand() % -34, rand() % 50346, rand() % 225));
		memory::write<Vector4>(entity + 0x2C, Vector4(rand() % 456, rand() % -25, rand() % 53460, rand() % -25));
		memory::write<Vector4>(entity + 0x48, Vector4(rand() % 100, rand() % -100, rand() % 50, rand() % 25));
		memory::write<Vector4>(entity + 0x48, Vector4(rand() % 100, rand() % -100, rand() % 50, rand() % 25));
		memory::write<Vector4>(entity + 0x48, Vector4(rand() % 345, rand() % -35, rand() % 35, rand() % 324));
		memory::write<Vector4>(entity + 0x48, Vector4(rand() % 2345, rand() % -14, rand() % 346, rand() % 25));
		memory::write<Vector4>(entity + 0x48, Vector4(rand() % 25, rand() % -34, rand() % 50346, rand() % 225));
		memory::write<Vector4>(entity + 0x48, Vector4(rand() % 456, rand() % -25, rand() % 53460, rand() % -25));

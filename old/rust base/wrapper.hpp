...
uintptr_t gameAssemblyTwo = LI_MODULE(("GameAssembly.dll")).get<uintptr_t>();

#define ProcAddr(func) LI_FN(GetProcAddress).cached()((HMODULE)gameAssemblyTwo, func)
//#define ProcAddr(func) GetProcAddress((HMODULE)gameAssemblyTwo, func)
...
namespace System {
	...
	/*template<typename T = RaycastHit*>
	struct STW {
	public:
		char pad_0000[0x10];
		void* buffer;

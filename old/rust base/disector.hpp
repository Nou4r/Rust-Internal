...
Il2CppClass* klass(uint32_t path) {
	if (stdafx::map_contains_key(offsset::offsets, path))
		return reinterpret_cast<Il2CppClass*>(offsset::offsets[path]);
	...
	for (int i = 0; i < domain->assemblyCount(); i++) {
		...
		for (int c = 0; c < image->classcount(); c++) {
			...
			Il2CppClass* pklass = image->get_class(c);
			char* name = pklass->name;
			char* ns = pklass->namespaze;
			...
			if (path == RUNTIME_CRC32(temp.c_str())) {
				uint64_t ptr = std::uint64_t(pklass);

				offsset::offsets.insert(std::make_pair(path, ptr));
				return pklass;
			}
		}
	}
...
uint64_t static_field(uint32_t path) {
	if (stdafx::map_contains_key(offsset::offsets, path))
		return std::uint32_t(offsset::offsets[path]);
	...
	for (int i = 0; i < domain->assemblyCount(); i++) {
		...
		for (int c = 0; c < image->classcount(); c++) {
			...
			Il2CppField* field;
			void* iter = NULL;
			while (field = pklass->fields(&iter)) {
				if (!field) continue;
				...
				if (RUNTIME_CRC32(t.c_str()) == path) {
					...
					uint64_t ptr = *reinterpret_cast<uint64_t*>(std::uint64_t(pklass->static_fields) + off);
					offsset::offsets.insert(std::make_pair(path, ptr));
	...
	for (int i = 0; i < domain->assemblyCount(); i++) {
		...
		for (int c = 0; c < image->classcount(); c++) {
			...
			while (field = pklass->fields(&iter)) {
				...
				if (RUNTIME_CRC32(t.c_str()) == path) {
					uint32_t off = field->offset();
					offsset::offsets.insert(std::make_pair(path, off));
...
uint64_t method(uint32_t path) {
	...
	for (int i = 0; i < domain->assemblyCount(); i++) {
		...
		for (int c = 0; c < image->classcount(); c++) {
			...
			Il2CppMethod* mthd;
			void* iter = NULL;
			while (mthd = pklass->methods(&iter)) {
				if (!mthd) continue;
				...
				if (RUNTIME_CRC32(temp2.c_str()) == path) {
					offsset::offsets.insert(std::make_pair(path, std::uint64_t(mthd)));
...
void init_classes() {
	Il2CppDomain* domain = il2cpp_domain_get();
	Il2CppAssembly** assemblies = domain->assemblies();
	printf("Count: %u\n", domain->assemblyCount());
	for (int i = 0; i < domain->assemblyCount() - 1; i++) {
		Il2CppImage* image = *reinterpret_cast<Il2CppImage**>(*reinterpret_cast<uint64_t*>(std::uint64_t(assemblies) + (0x8 * i)));
		for (int c = 0; c < image->classcount(); c++) {
			std::string temp(image->assemblyName);
			temp.erase(temp.find(xorstr_(".dll")), 4);
			...
			offsset::offsets.insert(std::make_pair(RUNTIME_CRC32(temp.c_str()), ptr));
			printf("Completed class count %u\n", c);
		}
		printf("Completed asm count %u of %u\n", i, domain->assemblyCount());
	}
}
void init_methods() {
	Il2CppDomain* domain = il2cpp_domain_get();
	...
	for (int i = 0; i < domain->assemblyCount() - 1; i++) {
		...
		for (int c = 0; c < image->classcount(); c++) {
			...
			while (mthd = pklass->methods(&iter)) {
				...
				offsset::offsets.insert(std::make_pair(RUNTIME_CRC32(temp2.c_str()), std::uint64_t(mthd)));
			}
		}
	}
}
	...
	for (int i = 0; i < domain->assemblyCount() - 1; i++) {
		...
		for (int c = 0; c < image->classcount(); c++) {
			...
			while (field = pklass->fields(&iter)) {
				...
				std::string t(temp + xorstr_("::") + field->name());
				uint32_t off = field->offset();
				offsset::offsets.insert(std::make_pair(RUNTIME_CRC32(t.c_str()), off));
			}
		}
	}
}
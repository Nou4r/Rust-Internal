...
/*Vector3 aimutils::Simulate(bool returnvelocity) {
    ...
    //Projectile* held = itemModProjectile->projectileObject()->Get()->GetComponent<Projectile>(Type::Projectile());
    rust::Projectile* held = reinterpret_cast<rust::Projectile*>(variables::local_player->get_held_item());
    if (held) {
        Vector3 pos = held->swimScale();
        Vector3 prev = held->tumbleAxis();
        float part = held->tumbleSpeed();
        float travel = Mathf::Max(held->traveledTime() - held->closeFlybyDistance(), 0);

        Vector3 gr = Physics::get_gravity(); //static Vector3 get_gravity();


        Vector3 origin = SimulateProjectile(pos, prev, part, travel, gr * held->gravityModifier(), held->drag());
